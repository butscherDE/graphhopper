...
public class Polygon implements Shape {
    ...
    public boolean contains(double lat, double lon) {
        if (earlyStoppingOnBoundingBox(lat, lon)) {
            return false;
        }

        final CrossProductRedBlueSegmentIntersection intersectionCounter = createIntersectionCounter(lat, lon);
        return isIntersectionCountUneven(intersectionCounter);
    }

    private boolean earlyStoppingOnBoundingBox(double lat, double lon) {
    ...
    private CrossProductRedBlueSegmentIntersection createIntersectionCounter(double lat, double lon) {
        final LineSegment ray = createRay(lat, lon);

        final List<LineSegment> rayAsLineSegmentList = Arrays.asList(new LineSegment[] {ray});
        final List<LineSegment> polygonLineSegments = this.getLineSegmentRepresentation();

        return new CrossProductRedBlueSegmentIntersection(rayAsLineSegmentList, polygonLineSegments);
    }

    private LineSegment createRay(double lat, double lon) {
        double rayStartLat = maxLat - (minLat / 2);
        double rayStartLon = minLon - epsilon;

        return new LineSegment(rayStartLon, rayStartLat, lon, lat);
    }

    private boolean isIntersectionCountUneven(CrossProductRedBlueSegmentIntersection intersectionCounter) {
        return (intersectionCounter.getIntersectionCount() % 2) == 1;
    }
